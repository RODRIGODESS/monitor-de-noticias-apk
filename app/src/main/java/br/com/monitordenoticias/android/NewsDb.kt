...
class NewsDb(context: Context) : SQLiteOpenHelper(context, "news.db", null, 2) {
    override fun onCreate(db: SQLiteDatabase) {
        ...
        db.execSQL("CREATE TABLE demands(id INTEGER PRIMARY KEY AUTOINCREMENT,vehicle TEXT NOT NULL,subject TEXT NOT NULL,active INTEGER DEFAULT 1)")
    ...
    fun listDemands(): List<Demand> = readableDatabase.rawQuery("SELECT id,vehicle,subject,active FROM demands ORDER BY id DESC",null).use { c -> buildList { while(c.moveToNext()) add(Demand(c.getLong(0),c.getString(1),c.getString(2),c.getInt(3)==1)) } }
    fun addDemand(vehicle: String, subject: String) { if(vehicle.isNotBlank() && subject.isNotBlank()) writableDatabase.execSQL("INSERT INTO demands(vehicle,subject) VALUES(?,?)", arrayOf(vehicle.trim(),subject.trim())) }
