...
class DesktopController(
    ...
    @Volatile var newsProgress = LiveSearchProgress()
    @Volatile var demandProgress = LiveSearchProgress()
    @Volatile var videoProgress = LiveSearchProgress()
    ...
    var startWithWindows: Boolean
        get() = prefs.getBoolean("desktop_start_with_windows", true)
        set(v) { prefs.edit().putBoolean("desktop_start_with_windows", v).apply(); updateWindowsStartup(v) }
    ...
    fun exportNewsHistoryCsv(): File {
        ...
        file.bufferedWriter(Charsets.UTF_8).use { w ->
            ...
            rows.forEach { n ->
                w.write(listOf(formatCsvDate(n.date), n.source, n.title, n.link, n.snippet, n.matchedTerm, n.matchedDemand).joinToString(";") { csv(it) })
                w.newLine()
            }
        }
    ...
    fun setNewsSource(id: String, selected: Boolean) {
        selectedNewsSourceIds = selectedNewsSourceIds.toMutableSet().apply { if (selected) add(id) else remove(id) }
    ...
    fun parsePeriod(sd: String, st: String, ed: String, et: String): Pair<Long, Long>? = runCatching {
        ...
        val start = LocalDateTime.of(parseDate(sd), LocalTime.parse(st.ifBlank { "00:00" })).atZone(zone).toInstant().toEpochMilli()
        val end = LocalDateTime.of(parseDate(ed), LocalTime.parse(et.ifBlank { "23:59" })).atZone(zone).toInstant().toEpochMilli()
        require(end > start)
        start to end
    }.getOrNull()
    ...
    fun newsAutoReport() = AutoReport(
        prefs.getLong(KEY_NEWS_ATTEMPT, 0L), prefs.getLong(KEY_NEWS_COMPLETED, 0L),
        prefs.getInt(KEY_NEWS_FOUND, 0), prefs.getInt(KEY_NEWS_NEW, 0), prefs.getInt(KEY_NEWS_ERRORS, 0),
        prefs.getString(KEY_NEWS_ERROR, "").orEmpty()
    )
    fun demandAutoReport() = AutoReport(
        prefs.getLong(KEY_DEMAND_ATTEMPT, 0L), prefs.getLong(KEY_DEMAND_COMPLETED, 0L),
        prefs.getInt(KEY_DEMAND_FOUND, 0), prefs.getInt(KEY_DEMAND_NEW, 0), prefs.getInt(KEY_DEMAND_ERRORS, 0),
        prefs.getString(KEY_DEMAND_ERROR, "").orEmpty(), prefs.getInt(KEY_DEMAND_CHECKED, 0)
    )
    fun videoAutoReport() = AutoReport(
        prefs.getLong(KEY_VIDEO_ATTEMPT, 0L), prefs.getLong(KEY_VIDEO_COMPLETED, 0L),
        prefs.getInt(KEY_VIDEO_FOUND, 0), prefs.getInt(KEY_VIDEO_NEW, 0), prefs.getInt(KEY_VIDEO_ERRORS, 0),
        prefs.getString(KEY_VIDEO_ERROR, "").orEmpty(), relevant = prefs.getInt(KEY_VIDEO_RELEVANT, 0)
    )

    private fun markAutoAttempt(kind: String) {
        val keyAttempt = when (kind) { "news" -> KEY_NEWS_ATTEMPT; "demand" -> KEY_DEMAND_ATTEMPT; else -> KEY_VIDEO_ATTEMPT }
        val keyError = when (kind) { "news" -> KEY_NEWS_ERROR; "demand" -> KEY_DEMAND_ERROR; else -> KEY_VIDEO_ERROR }
        prefs.edit().putLong(keyAttempt, System.currentTimeMillis()).putString(keyError, "").apply()
    }
    private fun markAutoFailed(kind: String, error: String) {
        val key = when (kind) { "news" -> KEY_NEWS_ERROR; "demand" -> KEY_DEMAND_ERROR; else -> KEY_VIDEO_ERROR }
        prefs.edit().putString(key, error.take(180)).apply()
    }
    private fun markNewsAutoCompleted(result: SearchResult) {
        prefs.edit().putLong(KEY_NEWS_COMPLETED, System.currentTimeMillis()).putInt(KEY_NEWS_FOUND, result.foundCount)
            .putInt(KEY_NEWS_NEW, result.newCount).putInt(KEY_NEWS_ERRORS, result.errors)
            .putString(KEY_NEWS_ERROR, if (result.errors > 0) "${result.errors} consulta(s) com falha" else "").apply()
    }
    private fun markDemandAutoCompleted(result: DemandSweepResult) {
        prefs.edit().putLong(KEY_DEMAND_COMPLETED, System.currentTimeMillis()).putInt(KEY_DEMAND_CHECKED, result.checkedCount)
            .putInt(KEY_DEMAND_FOUND, result.foundCount).putInt(KEY_DEMAND_NEW, result.newCount).putInt(KEY_DEMAND_ERRORS, result.errors)
            .putString(KEY_DEMAND_ERROR, if (result.errors > 0) "${result.errors} demanda(s) com falha" else "").apply()
    }
    private fun markVideoAutoCompleted(result: VideoSearchResult) {
        prefs.edit().putLong(KEY_VIDEO_COMPLETED, System.currentTimeMillis()).putInt(KEY_VIDEO_FOUND, result.foundCount)
            .putInt(KEY_VIDEO_NEW, result.newCount).putInt(KEY_VIDEO_RELEVANT, result.newRelevantCount).putInt(KEY_VIDEO_ERRORS, result.errors)
            .putString(KEY_VIDEO_ERROR, if (result.errors > 0) "${result.errors} fonte(s) instável(is)" else "").apply()
    }
    ...
    private fun updateWindowsStartup(enabled: Boolean) {
        ...
        runCatching {
            ...
            val command = if (enabled) listOf("reg", "add", key, "/v", "Monitor de Noticias", "/t", "REG_SZ", "/d", "\"$appPath\"", "/f")
            else listOf("reg", "delete", key, "/v", "Monitor de Noticias", "/f")
            ProcessBuilder(command).redirectErrorStream(true).start().apply { inputStream.bufferedReader().use { it.readText() }; waitFor() }
        }
    }
    ...
    private fun mergeNewsForUi(old: List<News>, fresh: List<News>) = (fresh + old).distinctBy { it.link }.sortedWith(compareByDescending<News> { it.capturedAt }.thenByDescending { it.date }).take(1500)
    private fun mergeVideosForUi(old: List<VideoItem>, fresh: List<VideoItem>) = (fresh + old).distinctBy { it.link }.sortedWith(compareByDescending<VideoItem> { it.capturedAt }.thenByDescending { it.publishedAt }).take(2000)
    ...
    data class AutoReport(val attemptAt: Long, val completedAt: Long, val found: Int, val newCount: Int, val errors: Int, val error: String, val checked: Int = 0, val relevant: Int = 0)
