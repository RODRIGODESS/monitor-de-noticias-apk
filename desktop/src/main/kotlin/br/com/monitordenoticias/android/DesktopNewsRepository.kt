...
import br.com.monitordenoticias.desktop.DesktopDirectNewsCollector
import br.com.monitordenoticias.desktop.DesktopSearchDiagnosticsStore
import br.com.monitordenoticias.desktop.DesktopSourceCatalog
...
/**
 * Implementação do contrato NewsRepository exclusiva do módulo Windows v4.0.3.
 *
 * Mantém o comportamento do motor compartilhado, acrescentando:
 * - aliases editoriais do catálogo Windows;
 * - identidade canônica de veículo para deduplicação;
 * - métricas de descarte/cobertura;
 * - segunda camada de coleta direta para fontes configuradas no Windows.
 *
 * O NewsRepository original do Android é excluído apenas da compilação desktop.
 */
class NewsRepository(private val db: NewsDb) {
    private val latestCollector = NewsLatestCollector()
    private val desktopDirectCollector = DesktopDirectNewsCollector()
    ...
    suspend fun searchAllDemands(): DemandSweepResult = withContext(Dispatchers.IO) {
        ...
        DemandSweepResult(
            ...
            fresh,
            errors,
            allItems.distinctBy { it.link }.sortedByDescending { it.date }
        )
    }
    ...
    private fun performSearch(
        ...
        DesktopSearchDiagnosticsStore.begin()
        try {
            ...
            tasks.forEach { task ->
                ...
                if (fetched == null) {
                    errors++
                    DesktopSearchDiagnosticsStore.recordGoogleError()
                } else {
                    val inPeriod = fetched.filter { it.date in from..to }
                    val sourceAccepted = inPeriod.filter { news ->
                        searchAllSources || selectedSources.any { selected -> sourceMatchesStrict(news.source, selected) }
                    }
                    DesktopSearchDiagnosticsStore.recordGoogle(
                        raw = fetched.size,
                        inPeriod = inPeriod.size,
                        sourceAccepted = sourceAccepted.size
                    )

                    val batch = sourceAccepted
                        .asSequence()
                        .map { base ->
                    ...
                    val mergedBatch = batch.map { incoming ->
                        val duplicateByStory = collected.values.firstOrNull { storyKey(it) == storyKey(incoming) }
                        when {
                            duplicateByStory != null -> mergeNews(
                                duplicateByStory,
                                incoming.copy(link = duplicateByStory.link, source = duplicateByStory.source)
                            )
                            collected[incoming.link] != null -> mergeNews(collected.getValue(incoming.link), incoming)
                            else -> incoming
                        }
                    }.distinctBy { it.link }
            ...
            // Coleta direta nacional já existente no motor compartilhado.
            ...
            DesktopSearchDiagnosticsStore.finish(items.size, newLinks.size)
            return SearchResult(items, items.size, newLinks.size, newDemandCount, errors)
        } catch (t: Throwable) {
            DesktopSearchDiagnosticsStore.fail()
            throw t
        }
    }

    private fun storyKey(news: News): String {
        val canonicalPublisher = DesktopSourceCatalog.canonicalName(news.source)
        val sourceKey = normalize(canonicalPublisher)
            .replace(" noticias", "")
            .replace(" jornal", "")
            .trim()
    ...
    private fun demandVehicleMatches(actualSource: String, vehicle: String): Boolean {
        ...
        val canonical = DesktopSourceCatalog.canonicalName(actualSource)
        val actual = normalize(canonical)
        val actualKey = compact(canonical)
        ...
        return tokens.size >= 2 && wantedKey.length >= 5 &&
            (actualKey.startsWith(wantedKey) || wantedKey.startsWith(actualKey) || hostKey.startsWith(wantedKey))
    }
    ...
    private fun sourceMatchesStrict(actualSource: String, selected: MediaSource): Boolean {
        if (DesktopSourceCatalog.publisherMatches(actualSource, selected)) return true
        ...
        return (listOf(selected.name) + selected.aliases).any { candidate ->
            ...
            if (candidateNormalized.isBlank() || candidateKey.isBlank()) false
                ...
                val meaningfulTokens = candidateNormalized.split(' ')
                    .filter { it.length >= 2 && it !in STOP_WORDS }
                meaningfulTokens.size >= 2 && candidateKey.length >= 6 &&
    ...
    private fun fetchGoogleNews(query: String): List<News>? = runCatching {
        ...
        val con = (url.openConnection() as HttpURLConnection).apply {
            ...
            readTimeout = 10000
            requestMethod = "GET"
            setRequestProperty("User-Agent", "Mozilla/5.0 MonitorNoticiasWindows/4.0.3")
